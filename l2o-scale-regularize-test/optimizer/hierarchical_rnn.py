...
class HierarchicalRNN(opt.TrainableOptimizer):
  ...
  def set_grad_flag_list(self, grad_flag_list):
    self.grad_flag_list = grad_flag_list
  ...
  def _compute_updates(self, params, grads, states, global_state):
    ...
    with tf.variable_scope(opt.OPTIMIZER_SCOPE):
      ...
      for _param, _grad_unflat, _state, _rd_ratio, _grad_flag in zip(params, grads, states, itertools.cycle(self.random_sparse_prob), self.grad_flag_list):
        # print("param", type(param), param.shape)
        # print("grad_unflat", type(grad_unflat), grad_unflat.shape)
        # for k in state:
        #   print(k,type(state[k]), state[k].shape)
        
        # if self.reuse_vars is None:
        #   reuse_flag = tf.AUTO_REUSE
        # else:
        #   reuse_flag = self.reuse_vars
        with tf.variable_scope("PerTensor", reuse=tf.AUTO_REUSE):
          ...
          print("_grad", type(_grad), _grad.shape)
          print("_param_flat", type(_param_flat), _param_flat.shape)
          ...
          if self.random_sparse_method == "layer_wise":
            ...
            if self.random_sparse_method == "params_wise":
          ...
          def update_all():
            # assign temp variable to the variable to process
            grad = _grad
            param = _param
            state = _state
          ...
          def run_update():
            ...
            attention_delta) = self._compute_rnn_state_projections(
                state, new_param_state, grads_scaled)
            print("update_step", type(update_step), update_step.shape)
            ...
            if self.use_attention:
              ...
              attention_to_append = combine_updates(attention_to_append, rd_indices, 
                              ...
                              param_shape)
              # updated_attention.append(truth - attention_step)
              # updated_attention.append(attention_to_append)
            else:
              updated_param = param - update_step
              # convert updated_param
              updated_param = combine_updates(updated_param, rd_indices, 
          ...
          if self.random_sparse_method == "layer_wise":
            final_tensor_list = tf.cond(rd_flag, lambda: update_all(), lambda: copy_previous())
          else:
