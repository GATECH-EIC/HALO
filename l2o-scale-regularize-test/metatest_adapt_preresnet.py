...
tf.app.flags.DEFINE_boolean("adapt_cifar10_preresnet", False,
                            """Include cifar10 Convnet problems but deeper.""")
...
tf.app.flags.DEFINE_float("prune_percent", 0.7, "pruning ratio")
tf.app.flags.DEFINE_string("save_dir", None, "The dir head to save log and ckpt")
...
def main(_):
    ...
    if FLAGS.adapt_cifar10_preresnet:
        if FLAGS.prune_percent == 0.7:
            problems_and_data.extend(ps.adapt_cifar10_preresnet20_p70(batch_size=FLAGS.batch_size, random_sparse_method=FLAGS.random_sparse_method, random_sparse_prob=prob_float_list))
        elif FLAGS.prune_percent == 0.8:
            problems_and_data.extend(ps.adapt_cifar10_preresnet20_p80(batch_size=FLAGS.batch_size, random_sparse_method=FLAGS.random_sparse_method, random_sparse_prob=prob_float_list))
        elif FLAGS.prune_percent == 0.9:
            problems_and_data.extend(ps.adapt_cifar10_preresnet20_p90(batch_size=FLAGS.batch_size, random_sparse_method=FLAGS.random_sparse_method, random_sparse_prob=prob_float_list))
    ...
    for problem_itr, (problem, dataset, batch_size) in enumerate(problems_and_data):
        ...
        if FLAGS.random_sparse_method is not None:
            save_dir = os.path.join(current_dir, FLAGS.save_dir, problem_name, FLAGS.test_optimizer +"_"+problem_feature+"_"+str(FLAGS.batch_size)+"_prune_"+str(FLAGS.prune_percent)+"_"+FLAGS.random_sparse_method+prob_dir_list)
        else:
            save_dir = os.path.join(current_dir, FLAGS.save_dir, problem_name, FLAGS.test_optimizer +"_"+problem_feature+"_"+str(FLAGS.batch_size)+"_prune_"+str(FLAGS.prune_percent))
        ...
        for seed in range(0, 20, 2):
            ...
            objective_values, acc_records, parameters, time_d = metaopt.test_optimizer(
                ...
                include_is_training = True if FLAGS.adapt_cifar10_preresnet else False)
