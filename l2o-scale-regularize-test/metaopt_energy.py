...
from problems.datasets import batch_indices, lstm_sin, lstm_UCI_HAR_Dataset, lstm_WISDM_HAR_Dataset
...
def test_optimizer(optimizer,
  ...
  if "lstm" in dataset:
    ...
    csv_file = os.path.join(save_dir, "energy.csv")
    if os.path.isfile(csv_file):
      os.remove(csv_file)
    def monitor_energy():
      tx2pwr.to_csv(csv_file, pwr_devices)
      monitor_thd = thd.Timer(1, monitor_energy)
      monitor_thd.daemon = True
      monitor_thd.start()
    ...
    with tf.Session(graph=graph, config=config) as sess:
      ...
      coord.request_stop()
      coord.join(threads)
      print("1 epoch: ", (run_end-run_start)/num_epochs)
    return np.mean(np.array(avg_iter_time))
