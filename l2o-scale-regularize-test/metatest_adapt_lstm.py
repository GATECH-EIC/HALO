...
tf.app.flags.DEFINE_string("train_dir", "adapt_conv_lstm/",
...
tf.app.flags.DEFINE_boolean("adapt_cifar10_resnet18", False,
                            """Include cifar10 Convnet problems but deeper.""")

######################################################################

tf.app.flags.DEFINE_boolean("adapt_lstm_sin", False,
                            """Include cifar10 Convnet problems but deeper.""")
tf.app.flags.DEFINE_boolean("adapt_conv_lstm", False,
                            """Include conv-list.""")
tf.app.flags.DEFINE_boolean("adapt_conv_lstm_5_layer", False,
                            """Include conv-list.""")
tf.app.flags.DEFINE_boolean("adapt_conv_UCI", False,
                            """Include conv-list.""")
tf.app.flags.DEFINE_boolean("adapt_conv_lstm_SML", False,
                            """Include conv-list.""")
...
tf.app.flags.DEFINE_float("lr", 1e-3, "learning rate for Adam/Adagrad/SGD")


# new added by hy
tf.app.flags.DEFINE_string("save_dir", "records_lstm",
                          """The minimum initial learning rate.""")
...
def main(_):
    ...
    if FLAGS.adapt_cifar10_resnet18:
        problems_and_data.extend(ps.adapt_cifar10_resnet18(batch_size=FLAGS.batch_size, random_sparse_method=FLAGS.random_sparse_method, random_sparse_prob=prob_float_list))

    if FLAGS.adapt_lstm_sin:
        problems_and_data.extend(ps.adapt_sin_lstm_problems(batch_size=FLAGS.batch_size, random_sparse_method=FLAGS.random_sparse_method, random_sparse_prob=prob_float_list))

    if FLAGS.adapt_conv_lstm:
        problems_and_data.extend(ps.adapt_conv_lstm_problems(batch_size=FLAGS.batch_size, random_sparse_method=FLAGS.random_sparse_method, random_sparse_prob=prob_float_list))

    if FLAGS.adapt_conv_lstm_5_layer:
        problems_and_data.extend(ps.adapt_conv_lstm_5_layer_problems(batch_size=FLAGS.batch_size, random_sparse_method=FLAGS.random_sparse_method, random_sparse_prob=prob_float_list))

    if FLAGS.adapt_conv_UCI:
        problems_and_data.extend(ps.adapt_conv_UCI_problems(batch_size=FLAGS.batch_size, random_sparse_method=FLAGS.random_sparse_method, random_sparse_prob=prob_float_list))

    if FLAGS.adapt_conv_lstm_SML:
        problems_and_data.extend(ps.adapt_conv_lstm_SML(batch_size=FLAGS.batch_size, random_sparse_method=FLAGS.random_sparse_method, random_sparse_prob=prob_float_list))
    ...
    for problem_itr, (problem, dataset, batch_size) in enumerate(problems_and_data):
        ...
        if FLAGS.random_sparse_method is not None:
            save_dir = os.path.join(current_dir, FLAGS.save_dir, problem_name, FLAGS.test_optimizer +"_"+problem_feature+"_"+str(FLAGS.batch_size)+"_"+FLAGS.random_sparse_method+prob_dir_list)
        else:
            save_dir = os.path.join(current_dir, FLAGS.save_dir, problem_name, FLAGS.test_optimizer +"_"+problem_feature+"_"+str(FLAGS.batch_size))
        ...
        for seed in range(0, 20, 2):
            ...
            training_loss_records, testing_loss_records, testing_acc_records, last_itr_loss , parameters, epoch_acc, time_d = metaopt.test_optimizer(
                opt,
                problem,
                ...
                include_is_training = True if FLAGS.adapt_cifar10_resnet18 else False)
            last_itr_losses.append(last_itr_loss)
            time_list.append(time_d)
            with open(os.path.join(save_dir, 'seed{}_train_loss_record.pickle'.format(seed)), 'wb') as l1_record:
                pickle.dump(training_loss_records, l1_record)
            with open(os.path.join(save_dir, 'seed{}_eval_loss_record.pickle'.format(seed)), 'wb') as record:
                pickle.dump(testing_loss_records, record)
            with open(os.path.join(save_dir, 'seed{}_eval_acc_record.pickle'.format(seed)), 'wb') as l2_record:
                pickle.dump(testing_acc_records, l2_record)
            with open(os.path.join(save_dir, 'seed{}_model_params_acc-{}.pickle'.format(seed, epoch_acc)), 'wb') as l3_record:
        ...
        ls_records['all'] = last_itr_losses
        mean = sum(last_itr_losses) / len(last_itr_losses)
        print('average loss :', mean)
        ls_records['mean'] = mean
        js = json.dumps(last_itr_losses, indent=1)
        with open(os.path.join(save_dir, 'last_itr_losses.txt'), 'w') as last_epoch_records_file:
            last_epoch_records_file.write(js)
    return 0
