...
tf.app.flags.DEFINE_integer("mini_batch_size", 8000,
                            """mini batch size.""")
...
tf.app.flags.DEFINE_boolean("set_profiling", True,
...
# regularization
tf.app.flags.DEFINE_string("regularize_time", 'posterior',
                            """When to add the regularization to the optimizer training""")
tf.app.flags.DEFINE_float("reg_scale", 0.5,
                          """The regularize scale for meta regularization.""")
...
def train_optimizer(logdir,
  ...
  for problem_itr, (problem_spec, dataset, batch_size) in enumerate(sampler):
    ...
    with graph.as_default(), graph.device(device_setter):
      ...
      train_output = opt.train(problem)
      ...
      with sv.managed_session(config=config) as sess:
        ...
        for k in range(num_meta_iterations):
          ...
          # if k < (num_meta_iterations // 2 + 1):
          #   # jacob_switch = tf.constant(True, tf.bool)
          #   jacob_switch = True
          # else:
          #   jacob_switch = False
          ...
          # reset optimizee's weight
          problem.init_fn(sess)

          if use_rd_scale:
            scale_np = []
            for k_id in range(len(train_output.scale_ph)):
              if (k_id == len(train_output.scale_ph)-1) and (train_output.scale_ph[k_id].shape[0] == aux_convex_dim):
                print("Find aux convex varible: ", train_output.scale_ph[k_id].shape)
                scale_np.append(np.exp(np.random.uniform(-aux_convex_rd_scale_bound, aux_convex_rd_scale_bound, 
                            size=train_output.scale_ph[k_id].shape)))
              else:
                scale_np.append(np.exp(np.random.uniform(-rd_scale_bound, rd_scale_bound, 
                            size=train_output.scale_ph[k_id].shape)))
          else:
            scale_np = [np.ones(shape=k.shape) for k in train_output.scale_ph]
          
          if train_output.fake_aux_convex_labels_ph_l1 is not None:
            fake_aux_convex_labels_np_l1 = np.random.uniform(-1.0, 1.0, size=train_output.fake_aux_convex_labels_ph_l1.shape)
          if train_output.fake_aux_convex_labels_ph_l2 is not None:
            fake_aux_convex_labels_np_l2 = np.random.uniform(-1.0, 1.0, size=train_output.fake_aux_convex_labels_ph_l2.shape)

          # set run options (for profiling)
          full_trace_opt = tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE)
          run_options = tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE, output_partition_graphs=True) if FLAGS.set_profiling else None
          # pdb.set_trace()
          # print(FLAGS.set_profiling)
          ...
          db = datasets.batch_indices(mini_batch_size, total_num_iter, batch_size)
          dataset_batches = []
          last_index = 0
          for num in partial_unroll_iters:
          ...
          # pdb.set_trace()
          # print(FLAGS.reg_scale)
          # print(FLAGS.regularize_time)
          for unroll_itr in range(num_unrolls):
            jacob_switch = True
            if FLAGS.regularize_time == 'posterior':
              
              if unroll_itr > int(num_unrolls * FLAGS.reg_scale + 1):
                jacob_switch = True
              else:
                jacob_switch = False
            elif FLAGS.regularize_time == 'prior':
              if unroll_itr < int(num_unrolls * FLAGS.reg_scale + 1):
                jacob_switch = True
              else:
                jacob_switch = False
            ...
            print('Total memory in system:%d.3GB' % zj)
            print('Allocated memory in system:%d.3GB' % (float(mem.used) / 1024 / 1024 / 1024))
            print('Free memory in system:%d.3GB' % (float(mem.free) / 1024 / 1024 / 1024))
            ...
            # mini_batch
            # pdb.set_trace()
            full_batch = np.arange(dataset.size)
            # print(dataset.size)
            np.random.shuffle(full_batch)
            mini_batch = full_batch[: mini_batch_size]
            ...
            feed = {
                ...
                train_output.jacob_switch : jacob_switch,
                train_output.mini_data : mini_data,
                train_output.mini_labels: mini_labels
            }
            extra_scale_dict = {p: v for p, v in zip(train_output.scale_ph, scale_np)}
            feed.update(extra_scale_dict)
            if train_output.fake_aux_convex_labels_ph_l2 is not None:
              feed[train_output.fake_aux_convex_labels_ph_l2] = fake_aux_convex_labels_np_l2
            if train_output.fake_aux_convex_labels_ph_l1 is not None:
              feed[train_output.fake_aux_convex_labels_ph_l1] = fake_aux_convex_labels_np_l1
            # print('系统已经使用内存:%d.3GB' % (float(mem.used) / 1024 / 1024 / 1024))
            # print('系统空闲内存:%d.3GB' % (float(mem.free) / 1024 / 1024 / 1024))
            ...
            with open("/tmp/run.txt", "w") as sys_out:
              sys_out.write(str(run_metadata))
            # print('系统已经使用内存:%d.3GB' % (float(mem.used) / 1024 / 1024 / 1024))
            # print('系统空闲内存:%d.3GB' % (float(mem.free) / 1024 / 1024 / 1024))
