...
tf.app.flags.DEFINE_boolean("adapt_cifar10_conv_problems", False,
                            """Include cifar10 Convnet problems.""")
tf.app.flags.DEFINE_boolean("adapt_cifar10_conv_problems_v2", False,
                            """Include cifar10 Convnet problems.""")
...
def main(_):
    ...
    if FLAGS.adapt_cifar10_conv_problems_v2:
        problems_and_data.extend(ps.adapt_cifar10_conv_problems_v2(batch_size=FLAGS.batch_size, random_sparse_method=FLAGS.random_sparse_method, random_sparse_prob=prob_float_list))
    ...
    for problem_itr, (problem, dataset, batch_size) in enumerate(problems_and_data):
        ...
        if FLAGS.random_sparse_method is not None:
            save_dir = os.path.join(current_dir, 'tmd', problem_name, FLAGS.test_optimizer +"_"+problem_feature+"_"+FLAGS.random_sparse_method+prob_dir_list)
        else:
