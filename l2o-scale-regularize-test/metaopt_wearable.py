...
from problems.datasets import batch_indices
...
def test_optimizer(optimizer,
  ...
  with tf.Session(graph=graph) as sess:
    ...
    test_batch_inds = batch_indices(test_imgs, test_labels, test_num_iter, batch_size, shuffle=False)
    ...
    for ei in range(num_epochs):
      ...
      train_batch_inds = batch_indices(train_imgs, train_labels, train_num_iter, batch_size, shuffle=False)
      record_length = int(train_num_iter/record_num)
      for itr, batch in enumerate(train_batch_inds):
        ...
        if itr%record_length == 0:
          print('itr{}-loss:'.format(itr), objective_values[itr])

          acc = []
          for _itr, batch in enumerate(test_batch_inds):
          ...
          itr_acc = sum(acc) / len(acc)
          print('itr{}-acc:'.format(itr), itr_acc)
          # acc_records.extend(acc)
          acc_records.append(itr_acc)
