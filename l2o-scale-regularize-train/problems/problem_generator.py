...
tf.app.flags.DEFINE_integer("hessian_itrs", 10, "max itrs for hessian")                       
...
class SoftmaxClassifier(Problem):
  ...
  def objective(self, params, data, labels, convex_l1_ratio=0.1,
                aux_conv_labels_l1=None, aux_conv_labels_l2=None):
    ...
    # add reg
    objs = tf.reduce_mean(losses) + tf.reduce_mean(l2reg) * FLAGS.l2_reg_scale
    # aux convex l2 norm
    if aux_conv_labels_l2 is not None:
      objs += self.aux_convex_ratio*tf.reduce_mean(tf.clip_by_value(tf.square(aux_conv_labels_l2 - params[-1]), 0, 10))
    # aux convex l1 norm
    if aux_conv_labels_l1 is not None:
      objs += self.aux_convex_ratio*convex_l1_ratio*tf.reduce_mean(tf.clip_by_value(tf.abs(aux_conv_labels_l1 - params[-1]), 0, 10))
...
class ConvNet(SoftmaxClassifier):
  ...
  def __init__(self,
               ...
               use_aux_convex=False,
               aux_convex_dim=20,
               aux_convex_ratio=1.0,
               random_seed=None,
               noise_stdev=0.0):
    ...
    # Add aux convex params if possible
    self.use_aux_convex = use_aux_convex
    self.aux_convex_dim = aux_convex_dim
    self.aux_convex_ratio = aux_convex_ratio
    
    if self.use_aux_convex:
      param_shapes.append((self.aux_convex_dim,))
  ...
  def inference(self, params, data):
    ...
    if self.use_aux_convex:
      ...
      main_param = params
    w_conv_list = main_param[:-2]
    output_w, output_b = main_param[-2:]
