      ...
      with sv.managed_session() as sess:
        ...
        for k in range(num_meta_iterations):
          ...
          for unroll_itr in range(num_unrolls):
            ...
            print('系统已经使用内存:%d.3GB' % (float(mem.used) / 1024 / 1024 / 1024))
            print('系统空闲内存:%d.3GB' % (float(mem.free) / 1024 / 1024 / 1024))
            ...
            # print('tensor-num', len(tensor_name_list))
...
def test_optimizer(optimizer,
  ...
  # class 1,3,5,7,9
  elif dataset == 'mnist-A':
    train, test = load_mnist()
    train_imgs, train_labels = train
    test_imgs, test_labels = test

    data_train = dummy_dataset(train_labels, train_imgs)
    data_test = dummy_dataset(test_labels, test_imgs)
    data_train = convert_dataset(data_train, "adapt", num_classes=10)
    data_test = convert_dataset(data_test, "adapt", num_classes=10)
    train_imgs, train_labels = data_train.images, data_train.labels
    test_imgs, test_labels = data_test.images, data_test.labels

    train_imgs = train_imgs.reshape(-1, 28, 28, 1)
    test_imgs = test_imgs.reshape(-1, 28, 28, 1)

  # class 0,2,4,6,8
  elif dataset == 'mnist-B':
    train, test = load_mnist()
    train_imgs, train_labels = train
    test_imgs, test_labels = test
    ...
    data_train = dummy_dataset(train_labels, train_imgs)
    data_test = dummy_dataset(test_labels, test_imgs)
    data_train = convert_dataset(data_train, "pretrain", num_classes=10)
    data_test = convert_dataset(data_test, "pretrain", num_classes=10)
    train_imgs, train_labels = data_train.images, data_train.labels
    test_imgs, test_labels = data_test.images, data_test.labels
  ...
  # class 0,2,5,6,8
    ...
    train_imgs = train_imgs.reshape(-1, 32, 32, 3)
    test_imgs = test_imgs.reshape(-1, 32, 32, 3)
  elif dataset == 'TDD-Male':
    train_imgs = np.load("TDD/TDD-Male-X_train.npy")
    train_labels = np.load("TDD/TDD-Male-y_train.npy")
    test_imgs = np.load("TDD/TDD-Male-X_test.npy")
    test_labels = np.load("TDD/TDD-Male-y_test.npy")
  elif dataset == 'TDD-Female':
    train_imgs = np.load("TDD/TDD-Female-X_train.npy")
    train_labels = np.load("TDD/TDD-Female-y_train.npy")
    test_imgs = np.load("TDD/TDD-Female-X_test.npy")
    test_labels = np.load("TDD/TDD-Female-y_test.npy")
  elif dataset == 'PIE-A':
    train_imgs = np.load("PIE/train_data_A.npy")
    train_imgs = np.transpose(train_imgs, (0,2,3,1))
    train_labels = np.load("PIE/train_label_A.npy")
    test_imgs = np.load("PIE/test_data_A.npy")
    test_imgs = np.transpose(test_imgs, (0,2,3,1))
    test_labels = np.load("PIE/test_label_A.npy")
  elif dataset == 'PIE-B':
    train_imgs = np.load("PIE/train_data_B.npy")
    train_imgs = np.transpose(train_imgs, (0,2,3,1))
    train_labels = np.load("PIE/train_label_B.npy")
    test_imgs = np.load("PIE/test_data_B.npy")
    test_imgs = np.transpose(test_imgs, (0,2,3,1))
    test_labels = np.load("PIE/test_label_B.npy")
  elif dataset == 'lstm-sin-G':
    n_l = problem.n_l
    train, test = lstm_sin(iterations=FLAGS.num_iterations,
                           n_batches=batch_size, n_l=n_l, noise_scale=FLAGS.noise_scale,
                           noise_type="Gaussian")
    train_imgs, train_labels = train
    ...
    # test_labels = test_labels.reshape(-1, )
    data_placeholder = tf.placeholder(tf.float32, shape=(batch_size, n_l, 1))
    labels_placeholder = tf.placeholder(tf.float32, shape=(batch_size, 1))
  elif dataset == 'lstm-sin-U':
    n_l = problem.n_l
    train, test = lstm_sin(iterations=FLAGS.num_iterations,
                           n_batches=batch_size, n_l=n_l, noise_scale=FLAGS.noise_scale,
                           noise_type="Uniform")
    ...
    data_placeholder = tf.placeholder(tf.float32, shape=(batch_size, n_l, 1))
    labels_placeholder = tf.placeholder(tf.float32, shape=(batch_size, 1))
  elif dataset == 'lstm_UCI_HAR_Dataset':
    # n_l = problem.n_l
    train_imgs, train_labels, test_imgs, test_labels = lstm_UCI_HAR_Dataset(
                                  iterations=FLAGS.num_iterations,
                                  n_batches=batch_size, prefix='./problems/')
    print(test_labels)
    label_dict = {}
    ...
    print(label_dict)
    # exit()
    data_placeholder = tf.placeholder(tf.float32, shape=(batch_size, 128, 9))
    labels_placeholder = tf.placeholder(tf.float32, shape=(batch_size, 1))
  # B - dataset used to prepare pretrain models
  elif dataset == 'lstm_UCI_HAR_Dataset-B':
    # n_l = problem.n_l
    train_imgs, train_labels, test_imgs, test_labels = lstm_UCI_HAR_Dataset(
                                  iterations=FLAGS.num_iterations,
                                  n_batches=batch_size, prefix='./problems/')
    ...
    data_train = dummy_dataset(train_labels, train_imgs)
    data_test = dummy_dataset(test_labels, test_imgs)
    data_train = convert_dataset(data_train, "pretrain", num_classes=6)
    data_test = convert_dataset(data_test, "pretrain", num_classes=6)
    train_imgs, train_labels = data_train.images, data_train.labels
    test_imgs, test_labels = data_test.images, data_test.labels
    # recover the shape
    ...
    data_placeholder = tf.placeholder(tf.float32, shape=(batch_size, 128, 9))
    labels_placeholder = tf.placeholder(tf.float32, shape=(batch_size, 1))
  # A - dataset used to prepare pretrain models
  elif dataset == 'lstm_UCI_HAR_Dataset-A':
    # n_l = problem.n_l
    train_imgs, train_labels, test_imgs, test_labels = lstm_UCI_HAR_Dataset(
                                  iterations=FLAGS.num_iterations,
                                  n_batches=batch_size, prefix='./problems/')
    ...
    data_train = dummy_dataset(train_labels, train_imgs)
    data_test = dummy_dataset(test_labels, test_imgs)
    data_train = convert_dataset(data_train, "adapt", num_classes=6)
    data_test = convert_dataset(data_test, "adapt", num_classes=6)
    train_imgs, train_labels = data_train.images, data_train.labels
    test_imgs, test_labels = data_test.images, data_test.labels
    # recover the shape
    train_labels = train_labels.reshape(-1,1)
    ...
    data_placeholder = tf.placeholder(tf.float32, shape=(batch_size, 128, 9))
    labels_placeholder = tf.placeholder(tf.float32, shape=(batch_size, 1))
  elif dataset == 'lstm_WISDM_HAR_Dataset':
    train_imgs, train_labels, test_imgs, test_labels = lstm_WISDM_HAR_Dataset()
    print(test_labels)
    # exit()
    data_placeholder = tf.placeholder(tf.float32, shape=(batch_size, 180, 3))
    labels_placeholder = tf.placeholder(tf.float32, shape=(batch_size, 1))
  elif dataset == 'lstm_WISDM_HAR_Dataset-B':
    train_imgs, train_labels, test_imgs, test_labels = lstm_WISDM_HAR_Dataset()
    train_labels = train_labels.reshape(-1,)
    test_labels = test_labels.reshape(-1, )
    data_train = dummy_dataset(train_labels, train_imgs)
    ...
    data_placeholder = tf.placeholder(tf.float32, shape=(batch_size, 180, 3))
    labels_placeholder = tf.placeholder(tf.float32, shape=(batch_size, 1))
  elif dataset == 'lstm_WISDM_HAR_Dataset-A':
    train_imgs, train_labels, test_imgs, test_labels = lstm_WISDM_HAR_Dataset()
    train_labels = train_labels.reshape(-1,)
    test_labels = test_labels.reshape(-1, )
    data_train = dummy_dataset(train_labels, train_imgs)
    ...
    data_placeholder = tf.placeholder(tf.float32, shape=(batch_size, 180, 3))
    labels_placeholder = tf.placeholder(tf.float32, shape=(batch_size, 1))
  elif dataset == 'lstm_SML_Dataset-B':
    train_imgs, train_labels, test_imgs, test_labels = lstm_SML_Dataset()
    dim = train_imgs.shape[2]
    data_placeholder = tf.placeholder(tf.float32, shape=(batch_size, 10, dim))
    labels_placeholder = tf.placeholder(tf.float32, shape=(batch_size, 10))
  elif dataset == 'lstm_SML_Dataset-A':
    train_imgs, train_labels, test_imgs, test_labels = lstm_SML_Dataset()
    dim = train_imgs.shape[2]
    data_placeholder = tf.placeholder(tf.float32, shape=(batch_size, 10, dim))
    labels_placeholder = tf.placeholder(tf.float32, shape=(batch_size, 10))
    # exit()
  elif dataset == 'MLP_GAS_Dataset-B':
    train_imgs, train_labels, test_imgs, test_labels = MLP_GAS_Dataset()
    data_train = dummy_dataset(train_labels, train_imgs)
    data_test = dummy_dataset(test_labels, test_imgs)
    data_train = convert_dataset(data_train, "pretrain", num_classes=6)
    data_test = convert_dataset(data_test, "pretrain", num_classes=6)
    ...
    for i in labels_dict:
      print("label: {}, num: {}".format(i, labels_dict[i]))

    # train_labels = train_labels.reshape(-1,1)
    # test_labels = test_labels.reshape(-1,1)
    dim = train_imgs.shape[1]
    data_placeholder = tf.placeholder(tf.float32, shape=(batch_size, dim))
    labels_placeholder = tf.placeholder(tf.float32, shape=(batch_size, 1))
  elif dataset == 'MLP_GAS_Dataset-A':
    train_imgs, train_labels, test_imgs, test_labels = MLP_GAS_Dataset()
    data_train = dummy_dataset(train_labels, train_imgs)
    data_test = dummy_dataset(test_labels, test_imgs)
    data_train = convert_dataset(data_train, "adapt", num_classes=6)
    ...
    # print("X train size: ", train_imgs.shape)
    # print("X test size: ", test_imgs.shape)
    # print("y train size: ", train_labels.shape)
    # print("y test size: ", test_labels.shape)
    # print(test_labels[1:100])
    # exit()
  ...
  if "lstm" in dataset:
    ...
    with tf.Session(graph=graph, config=config) as sess:
      ...
      for ei in range(num_epochs):
        ...
        print('epoch{}-loss:'.format(ei), epoch_loss)
        acc = []
        for itr, batch in enumerate(test_batch_inds):
        ...
        epoch_acc = sum(acc) / len(acc)
        print('epoch{}-acc:'.format(ei), epoch_acc)
    ...
    return objective_values, acc_records, parameters, np.mean(np.array(avg_iter_time))
