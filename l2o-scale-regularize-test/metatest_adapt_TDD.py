...
tf.app.flags.DEFINE_boolean("adapt_cifar10_resnet18", False,
                            """Include cifar10 Convnet problems but deeper.""")
tf.app.flags.DEFINE_boolean("adapt_TDD_MLP_problems", False,
                            """Include cifar10 Convnet problems but deeper.""")
...
def main(_):
    ...
    if FLAGS.adapt_TDD_MLP_problems:
        problems_and_data.extend(ps.adapt_TDD_MLP_problems(batch_size=FLAGS.batch_size, random_sparse_method=FLAGS.random_sparse_method, random_sparse_prob=prob_float_list))
    ...
    for problem_itr, (problem, dataset, batch_size) in enumerate(problems_and_data):
        ...
        if FLAGS.random_sparse_method is not None:
            save_dir = os.path.join(current_dir, 'records_TDD', problem_name, FLAGS.test_optimizer +"_"+problem_feature+"_"+str(FLAGS.batch_size)+"_"+FLAGS.random_sparse_method+prob_dir_list)
        else:
            save_dir = os.path.join(current_dir, 'records_TDD', problem_name, FLAGS.test_optimizer +"_"+problem_feature+"_"+str(FLAGS.batch_size))
