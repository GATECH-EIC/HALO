...
class TrainableOptimizer(tf.train.Optimizer):
  ...
  def __init__(self, name, state_keys, use_attention=False,
               ...
               convex_l1_ratio=0.1, use_aux_convex_l1=False,
               use_aux_convex_l2=False, aux_convex_dim=20, pretrained_model_path=None,
               **kwargs):
  ...
  def train(self, problem):
    ...
    if self.use_aux_convex_l1:
      self.fake_aux_convex_labels_ph_l1 = tf.placeholder(tf.float32, shape=[self.aux_convex_dim])
    else:
      self.fake_aux_convex_labels_ph_l1 = None
    if self.use_aux_convex_l2:
      self.fake_aux_convex_labels_ph_l2 = tf.placeholder(tf.float32, shape=[self.aux_convex_dim])
    else:
      self.fake_aux_convex_labels_ph_l2 = None
    ...
    def loop_body(itr, obj_accum, params, attend_params, flattened_states,
      ...
      # add scale to the params and attend_params with calculate loss
      scaled_params = [params[k_id]*self.scale_ph[k_id] for k_id in range(len(self.scale_ph))]
      scaled_attend_params = [attend_params[k_id]*self.scale_ph[k_id] for k_id in range(len(self.scale_ph))]
      ...
      obj = problem.objective(scaled_params, mini_data, mini_labels, convex_l1_ratio=self.convex_l1_ratio,
                              aux_conv_labels_l1=self.fake_aux_convex_labels_ph_l1,
                              aux_conv_labels_l2=self.fake_aux_convex_labels_ph_l2)

      # Compute the gradients on just the current batch
      if self.use_attention:
        # current_obj = problem.objective(attend_params, batch_data, batch_labels)
        current_obj = problem.objective(scaled_attend_params, batch_data, batch_labels,
                                        convex_l1_ratio=self.convex_l1_ratio,
                                        aux_conv_labels_l1=self.fake_aux_convex_labels_ph_l1,
                                        aux_conv_labels_l2=self.fake_aux_convex_labels_ph_l2)
        grads, reg = problem.gradients(current_obj, attend_params)
      else:
        # current_obj = problem.objective(params, batch_data, batch_labels)
        current_obj = problem.objective(scaled_params, batch_data, batch_labels,
                                        convex_l1_ratio=self.convex_l1_ratio,
                                        aux_conv_labels_l1=self.fake_aux_convex_labels_ph_l1,
                                        aux_conv_labels_l2=self.fake_aux_convex_labels_ph_l2)
      ...
      return [itr + 1, obj_accum, new_params, new_attend_params,
              new_flattened_states, new_global_state, all_obj, unused_init_obj,
              mini_data, mini_labels, batches, regular]
    ...
    init = self._initialize_training_loop_parameters(
        problem, mini_data, mini_labels, batches, first_unroll, reset_state)
    ...
    meta_obj, problem_objectives, jacob = loop_output[1], loop_output[6], loop_output[-1]
    ...
    return training_output(scaled_meta_objective,
                           ...
                           mini_labels,
                           self.scale_ph,
                           self.fake_aux_convex_labels_ph_l1,
                           self.fake_aux_convex_labels_ph_l2)
  ...
  def _initialize_training_loop_parameters(
    ...
    # set the scale placeholder with the same shape of initial params
    self.scale_ph = []
    for k in initial_tensors:
      self.scale_ph.append(tf.placeholder(tf.float32, shape=k.shape, ))
    
    # change the initilization according to the scale generated
    for k_id in range(len(initial_tensors)):
      initial_tensors[k_id] = initial_tensors[k_id]/self.scale_ph[k_id]
