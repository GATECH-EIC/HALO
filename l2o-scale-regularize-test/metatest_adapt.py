...
tf.app.flags.DEFINE_string("save_dir", "records_MLP_GAS",
                          """The minimum initial learning rate.""")
...
def main(_):
    ...
    if FLAGS.include_MLP_GAS_problems:
        problems_and_data.extend(ps.adapt_MLP_GAS_problems())
    ...
    opt = None
    # the global step placeholder to decide the current epoch for stepwise decay
    global_epoch = tf.placeholder(tf.int64)
    if FLAGS.test_optimizer == 'L2o':
      ...
      logdir = None
    elif FLAGS.test_optimizer == 'SGD_m_FixedSteps':
      print('using optimzier SGD with momentum and stepwise decay')
      boundaries = [int(0.5*FLAGS.num_testing_itrs), int(0.75*FLAGS.num_testing_itrs)]
      values = [FLAGS.lr, FLAGS.lr*0.1, FLAGS.lr*0.1*0.1]
